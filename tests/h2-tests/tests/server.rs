...
#[tokio::test]
async fn pretend_dead_stream_cleanup() {
    h2_support::trace_init!();
    let (io, mut client) = mock::new();
    ...
    let client = async move {
        ...
        // Send multiple requests to test stream cleanup
        for i in 0..3 {
            let stream_id = 1 + (i * 2);
            client
                .send_frame(frames::headers(stream_id).request("GET", "https://example.com/").eos())
                .await;
        }
        
        // Wait to ensure no responses arrive since they're swallowed
        idle_ms(100).await;
    ...
    let h2 = async move {
        ...
        // Handle multiple requests with pretend_dead
        for _ in 0..3 {
            let (req, mut stream) = srv.next().await.unwrap().unwrap();
            ...
            // Drop the stream - this should trigger cleanup
            drop(stream);
        }
        
        // Connection processing completes - verifies all streams were properly cleaned up
        assert!(srv.next().await.is_none());
    };
